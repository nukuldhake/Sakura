...
import { CheckBar, IconItem } from '@proj-sakura/stage-ui/components'
...
<template>
  ...
  <CheckBar
    v-model="serverChannelStore.websocketEnabled"
    v-motion
    mb-4
    icon-on="i-solar:transfer-horizontal-bold-duotone"
    icon-off="i-solar:transfer-horizontal-linear"
    text="Enable local Mods server WebSocket connection"
    description="If enabled, this will attempt to connect to the local server or devtools. Disabling this avoids 'WebSocket connection failed' errors in console when no local server is running."
    :initial="{ opacity: 0, y: 10 }"
    :enter="{ opacity: 1, y: 0 }"
    :duration="250 + (21 * 10)"
    :delay="3 * 50"
