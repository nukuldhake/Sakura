...
こんにちは、SAKURA のメンテナの一人、LemonNeko です。今日の DevLog では、SAKURA デスクトップペットの Live2D モデルがマウスの位置を注視できるようにする方法についてお話しします。
...
![](/en/blog/DevLog-2025.06.08/assets/SAKURA-tamagotchi-focus.gif)
...
青い枠が画面、ピンクが SAKURA のウィンドウ、紫の矢印がマウスです。次のように定義します：

- 画面の高さと幅：`A x B`
- SAKURA ウィンドウの左上の位置：`(E, F)`
- SAKURA ウィンドウのサイズ：`C x D`
- マウスの位置：`G, H`

すると、SAKURA ウィンドウ内のマウスの位置は `(G - E, H - F)` になるはずです。
