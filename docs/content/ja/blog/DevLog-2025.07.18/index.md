...
date: 2025-07-18
excerpt: |
  Factorio 強化学習環境に関する論文を読んだ後、Factorio エージェントプロジェクト `SAKURA-factorio` をどのように改善するか共有したいと思います。
preview-cover:
...
こんにちは、SAKURA メンテナーの一人、[@LemonNeko](https://github.com/LemonNekoGH) です。

## 振り返り

半年前、私は有名な自動化生産シミュレーションゲーム [Factorio](https://www.factorio.com/) をプレイできる AI エージェント [`SAKURA-factorio`](https://github.com/moeru-ai/SAKURA-factorio) を書く最初の試みを行いました。その中で以下の実践を行いました：
...
ここまで読んで、彼らの評価がこれほど複雑であるなら、技術的な実装においてどのように保守性を保証しているのか興味を持ち始めました。読み進めると、彼らの実装方法は `SAKURA-factorio` と非常に似ていますが、`SAKURA-factorio` に比べて多くの利点があることがわかりました：
...
FLE と比較すると、私たちの実装はかなり未熟に見えます。では、どのように `SAKURA-factorio` を改善すればよいでしょうか？
...
読んでいただきありがとうございます。興味があれば、FLE の論文と [コード](https://github.com/JackHopkins/factorio-learning-environment) をご覧ください。私の理解が間違っているかもしれませんので、ご指摘いただければ幸いです！今回の読み込みは十分ではないかもしれませんが、今後私の考えに従って `SAKURA-factorio` を改善する際には、繰り返し読む必要があり、進展があれば更新します。
