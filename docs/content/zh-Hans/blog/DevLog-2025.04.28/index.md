...
一周前，我为 SAKURA 写了用于连接到手机的 MCP 服务器 [SAKURA-android](https://github.com/LemonNekoGH/SAKURA-android)，但这只是 SAKURA 操作安卓手机的前半部分，SAKURA 还需要能与 MCP 服务器交互才行。

这两天我完成了后半部分，给 Tauri 写了一个插件 [#144](https://github.com/moeru-ai/SAKURA/pull/144)，现在 SAKURA 可以与 MCP 服务器交互了，可以和现有的所有 MCP 服务器交互。

如果有兴趣，可以看看这两个视频，先演示了 SAKURA 的 MCP 服务器设置，然后演示了 SAKURA 与安卓手机交互。

<details>
  <summary>SAKURA 的 MCP 服务器设置</summary>
  <ThemedVideo controls muted src="./assets/SAKURA-mcp-settings.mp4" style="height: 640px;" />
</details>

<details>
  <summary>SAKURA 在手机上输入 `Hello World`</summary>
...
1. 从演示视频可以看到，在对话中，我首先是让 SAKURA 获取了一下工具列表，再让它输入文本的，那我们能不能在初始化的时候就去获取工具列表，然后直接追加到系统提示词中呢？
   - Cursor 就是这样做的，在我开发 MCP 服务器时，每次我改动了工具列表，都需要重启 Cursor 才能生效。
   - 这样做也许会牺牲灵活性，但普通用户会频繁改动工具列表吗？

2. 要允许 SAKURA 同时连接到多个手机吗？SAKURA 可能会想使用多台手机吗？~~她会不会想拿去做电信诈骗？~~
3. 可以看到现在的 SAKURA 仓库中已经有了 Tauri 应用和 Tauri 插件，要怎么管理比较好？CI 要怎么配置？如何同步 Tauri 插件的 Rust 侧和 JavaScript 侧的版本号？

## 未来想要做的事情

- 支持图片返回值，这样 SAKURA 就可以像 [上一篇 DevLog](./DevLog-2025.04.22.md) 中展示的 Cursor 那样，直接通过视觉能力看到手机上的内容，然后再决定用什么方式来交互。
- 让 SAKURA 自己学习设备的使用方法？如果每种设备我们都要单独写提示词，那工作量是巨大的。
- 多 MCP 服务器支持，毕竟 MCP 提供了一种通用的接口，可以允许 SAKURA 做各种各样的事，SAKURA 应该不会满足于只操作手机吧。
- SSE 支持，这样浏览器中的 SAKURA 也可以使用 MCP 服务器了。
