...
export function useVRMEmote(vrm: VRMCore) {
  ...
  // Emotion states definition — each emotion maps to a blend of VRM expressions
  // for richer, more natural-looking facial animation.
  const emotionStates = new Map<string, EmotionState>([
    ...
    ['think', {
      expression: [
        ...
        { name: 'blink', value: 0.15 }, // slight squint while thinking
      ],
      blendDuration: 0.5,
    }],
    // --- Extended emotions ---
    ...
    ['awkward', {
      expression: [
        { name: 'sad', value: 0.3 },
        { name: 'oh', value: 0.2 },
        { name: 'blink', value: 0.2 }, // nervous partial squint
      ],
      blendDuration: 0.4,
    }],
  ])
