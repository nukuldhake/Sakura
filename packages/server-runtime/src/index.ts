import type { MetadataEventSource, WebSocketEvent } from '@proj-sakura/server-shared/types'
...
import { MessageHeartbeat, MessageHeartbeatKind, WebSocketEventSource } from '@proj-sakura/server-shared/types'
...
export function setupApp(options?: {
  ...
  const appLogger = useLogg('@proj-sakura/server-runtime').withLogLevel(logLevelStringToLogLevelMap[appLogLevel]).withFormat(appLogFormat)
  const logger = useLogg('@proj-sakura/server-runtime:websocket').withLogLevel(logLevelStringToLogLevelMap[websocketLogLevel]).withFormat(websocketLogFormat)
